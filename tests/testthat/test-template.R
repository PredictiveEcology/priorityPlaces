...
#    `functionName` is a function's name in your module (e.g., `priorityPlacesEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Tati/Documents/GitHub/priorityPlaces/priorityPlaces/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("priorityPlaces")
  path <- list(modulePath = "C:/Users/Tati/Documents/GitHub/priorityPlaces",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    priorityPlaces = list(.saveInitialTime = NA)
  )
  ...
  if (exists("priorityPlacesEvent1", envir = .GlobalEnv)) {
    simOutput <- priorityPlacesEvent1(mySim)
  } else {
  ...
  if (exists("priorityPlacesEvent2", envir = .GlobalEnv)) {
    simOutput <- priorityPlacesEvent2(mySim)
  } else {
